      ...
      try {
        ...
        try {
          const {
            ...
            buildManageUrl,
          } = await import('@/lib/registration-manage-token')
          const manageToken = await createRegistrationManageToken(registration.id)
          const manageUrl = buildManageUrl(conference.slug, manageToken)
          const manageLabel =
          ...
          log.warn('Failed to create registration manage token', manageErr)
        }
...
View in admin panel: ${process.env.NEXT_PUBLIC_APP_URL || 'http://localhost:3000'}/admin/registrations
    ...
    const isPayNowCard = paymentMethod === 'card'
    let paymentPayload: {
