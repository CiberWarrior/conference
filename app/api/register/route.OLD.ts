...
const registrationSchema = z
  .object({
    ...
    participants: z
      ...
      .optional(),
    customFields: z.record(z.any()).optional(), // Custom fields for main registrant
  ...
  .refine(
    (data) => {
      ...
      if (data.participants && data.participants.length > 0) {
        return data.participants.every((participant) => {
          if (!participant.firstName || !participant.lastName || !participant.email) {
            return false
          }
          // Validate dates if provided
          if (participant.arrivalDate && participant.departureDate) {
            return new Date(participant.departureDate) >= new Date(participant.arrivalDate)
          }
          return true
        })
      }
    ...
    {
      message: 'All participants must have valid names, emails, and dates',
      path: ['participants'],
    }
  )
  ...
  try {
    ...
    const { data: registration, error: insertError } = await supabase
      ...
      .insert({
        ...
        custom_data: validatedData.customFields || {},
        participants: validatedData.participants || [], // Store multiple participants
      })
      .select()
      .single()
